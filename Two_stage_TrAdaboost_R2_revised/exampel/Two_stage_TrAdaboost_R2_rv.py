...
def Two_stage_TrAdaboost_R2_rv(trans_S, Multi_trans_A, response_S, Multi_response_A, test, steps_S, N):
    ...
    for i in range(steps_S):
        ...
        if len(model_error) > 2 and model_error[-1] > model_error[-2] and model_error[-1] > model_error[-3] and model_error[-1] > model_error[-4]:
            steps_S = i
            break
        ...
        total_w_S =  0.5 + 0.5 * i/(steps_S-1) # 0.5 + 0.5 * t / (S-1) t = 0, 1, ..., S-1
